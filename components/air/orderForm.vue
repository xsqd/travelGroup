<template>
  <div class="main">
    <div class="air-column">
      ...
      <el-form class="member-info" :model="{users}" ref="formUsers">
        <div class="member-info-item" v-for="(user,index) in users" :key="index">
          <el-form-item label="乘机人类型" :prop="`users.${index}.username`" :rules="{required: true,message: '请输入乘机人姓名',trigger: 'blur'
            }">
          ...
          <el-form-item label="证件类型" :prop="`users.${index}.id`" :rules="{required: true,message: '请输入乘机人证件号',trigger: 'blur'
            }">
    ...
    <div class="air-column">
      <h2>联系人</h2>
      <div class="contact">
        <el-form label-width="80px" :model="{contactName,contactPhone,captcha}" :rules="contactRules" ref="formContact">
...
export default {
  ...
  methods: {
    ...
    async handleSubmit() {
      ...
      const validUsers = await this.$refs.formUsers.validate().catch(()=>false)
      const validContact = await this.$refs.formContact.validate().catch(()=>false)
